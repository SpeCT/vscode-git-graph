...
* Git Graph View:
    ...
    * Repository Settings Widget:
        ...
        * Configure "Pull Request Creation" - Automates the opening and pre-filling of a Pull Request form, directly from a branches context menu.
            ...
            * For Bitbucket Cloud repositories, relevant Pull Requests are displayed beside branch labels with a compact lifecycle / review status indicator (e.g. merged, approved, changes requested, or awaiting review). After a closed Pull Request, adding new commits to the branch replaces the old badge with a button that opens Bitbucket's pre-filled creation page (except `develop`, `main`, `master`, `preprod`, and `production`).
            * Bitbucket Cloud pipeline badges appear beside commit messages, independently of branch labels. Hover for the latest run of each pipeline; click for all runs, including earlier attempts, with links to Bitbucket. Failures and running pipelines remain visible when several pipelines share a commit.
            * Pipeline data loads for visible commits. Active runs refresh about every 30 seconds, and other visible commits every two minutes; the graph Refresh button updates immediately. Pipeline badges currently show status and links; triggering, rerunning, and cancelling remain in Bitbucket.
            ...
            * Private Bitbucket Cloud repositories require an API token with **Pull requests: Read** permission. Pipeline status also requires **Pipelines: Read** (`read:pipeline:bitbucket`) permission. Use the `Git Graph: Set Bitbucket Cloud API Token...` command; the token is stored in Visual Studio Code's encrypted Secret Storage. Pull Requests are still created in the browser.
